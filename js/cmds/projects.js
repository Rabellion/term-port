export default async function (output, _hist, ...args) {
  if (args.length) {
    ...
    window.open('https://github.com/Rabellion/' + name);
  } else {
    let repositories;
    try {
      repositories = await fetch(
        "https://api.github.com/users/Rabellion/repos?type=owner&sort=created"
      ).then((r) => r.json());
    } catch (e) {
    ...
    output.innerHTML += `
View More On My <span><a href="https://github.com/Rabellion?tab=repositories">Github</a></span>\n
`;
  }
}
